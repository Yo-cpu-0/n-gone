        ...
        },
        {
                ...
                if (m.energy > DRAIN && this.canMRFire) {
                    ...
                    bullet[me] = Bodies.polygon(where.x + 20 * Math.cos(m.angle), where.y + 20 * Math.sin(m.angle), 4, 0.01, {
                        ...
                        do() {
                            ...
                            this.drawRadius = Math.max(Math.max(this.endCycle - simulation.cycle, 10) - 1, 0.1);
                            ...
                            this.cycle++;
                            const wiggleMag = (!m.alive ? 3 : 0) * Math.cos(Math.sin(simulation.cycle) * 0.09);
            ...
            do() {
                ...
                for (let i = bullet.length - 1; i > 0; i--) {
                    if (bullet[i].isWave2) {
                        if (bullet[i].isBranch || bullet[i - 1].isBranch || bullet[Math.max(i - 2, 0)].isBranch || bullet[Math.min(i + 1, bullet.length - 1)].isBranch) {
                            ...
                            ctx.strokeStyle = "rgba(0, 0, 0, 1)"; // Black color for the fluid
                            ctx.stroke();
                        }
                    }
                }
                ...
                /*for (let i = bullet.length - 1; i > 0; --i) {
                    if (bullet[i].isWave2) {
                        if (bullet[i].isBranch || bullet[i - 1].isBranch || bullet[Math.max(i - 2, 0)].isBranch || bullet[Math.min(i + 1, bullet.length - 1)].isBranch) {
                            ...
                            ctx.lineWidth = 2;
                            ctx.strokeStyle = "rgba(3, 244, 252, 0.1)";
                            ctx.stroke();
                        }
                    }
                }*/
                /*if(m.crouch) {
                    for (let i = bullet.length - 1; i > 0; --i) {
                        if(bullet[i].isWave2) {
                            bullet[i].force = Vector.mult(Vector.normalise(Vector.sub(bullet[i].position, simulation.mouseInGame)), -bullet[i].mass * 0.0001);
                            this.valueReduction = 0;
                            this.drawRadius = 1;
                            if (this.speed > 6) {
